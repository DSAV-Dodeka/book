...
To be able to download and upload repositories, you'll still need to login to GitHub. For that, I recommend using the `gh` CLI (GitHub CLI). Install it by following the instructions at [https://cli.github.com/](https://cli.github.com/). The `gh` CLI is also used by the backend to auto-download the auth binary, so it's required for backend development.
